...
class UserService:
    ...
    async def create_user(self, name: str, email: str, password: str, memo: str | None = None, role: Role = Role.USER) -> User:
        async with self.uow as uow:
            exists_user = await self.find_user_by_email(email)
            if exists_user: 
                raise HTTPException(422, detail=f"User already exists: {email}")
